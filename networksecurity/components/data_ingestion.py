...
class DataIngestion:
    ...
    def export_collection_as_dataframe(self):
        """
        Connect to MongoDB, retrieve the collection and convert it into a DataFrame
        """
        try:
            ...
            if df.shape[0] == 0:
                raise ValueError(f"No records found in MongoDB collection: {database_name}.{collection_name}")

            if "_id" in df.columns.to_list():
                df = df.drop(columns=["_id"], axis=1)

            df.replace({"na": np.nan}, inplace=True)
            logging.info(f"Exported {df.shape[0]} records from MongoDB collection: {database_name}.{collection_name}")
            return df
